...
from typing import Iterator, List, Optional # isort: skip
...
def run_job_sync(
    ...
    with _job_run_bookkeeping_context(job_name, job_args) as run_model:
        ...
        job_pipeline.run().wait_until_finish()
...
def run_job_async(
    ...
    """Runs the specified job asynchronously.
...
def _job_run_bookkeeping_context(
    job_name: str,
    job_args: List[str]
) -> Iterator[beam_job_models.BeamJobRunModel]:
    """Returns a context manager that manages and commits job-related models.

    Args:
        job_name: str. The name of the job.
        job_args: List[str]. The arguments to the job.

    Yields:
        beam_job_domain.BeamJobRun. The run associated to the context.
    """
