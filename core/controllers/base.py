...
class BaseHandler(
    ...
    def __init__(  # pylint: disable=super-init-not-called
        ...
        if auth_claims:
            ...
            if user_settings.deleted:
                ...
                and not user_services.has_fully_registered_account(self.user_id)
            ):
    ...
    def validate_and_normalize_args(self) -> None:
        ...
        extra_args_are_allowed = (
            self.GET_HANDLER_ERROR_RETURN_TYPE == feconf.HANDLER_TYPE_HTML
    ...
    def _is_requested_path_currently_accessible_to_user(self) -> bool:
        ...
        """
        return (
            self.request.path in AUTH_HANDLER_PATHS
            or not feconf.ENABLE_MAINTENANCE_MODE
            or self.current_user_is_site_maintainer
        )
    ...
    def handle_exception(
        ...
        if isinstance(exception, self.NotLoggedInException):
            ...
            self._log_exception_message(
                exception_type, LogType.WARNING, 'Unauthenticated user'
            )
            if (
                payload_exists
                or self.GET_HANDLER_ERROR_RETURN_TYPE
                == feconf.HANDLER_TYPE_JSON
            ):
                self.error(401)
                values: ResponseValueDict = {
        ...
        if isinstance(exception, self.NotFoundException):
            self._log_exception_message(
                exception_type, LogType.WARNING, 'Invalid URL requested'
            )
            self.error(404)
            values = {
                'error': 'Could not find the resource %s.' % self.request.uri,
