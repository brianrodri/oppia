...
class FirebaseAuditRecordsJob(base_jobs.JobBase):
    ...
    def _audit_grouped_records(
        ...
        if len(weak_entries := set(grouped['weak_entries'])) > 1:
            collisions_found = True
            ids = sorted(record.user_id for record in weak_entries)
            yield pvalue.TaggedOutput(
                self.TAG_ERR, f'Oppia User IDs have same {email=}: {ids!r}'
            )

        if len(strong_entries := set(grouped['strong_entries'])) > 1:
            collisions_found = True
            ids = sorted(record.auth_id for record in strong_entries)
            yield pvalue.TaggedOutput(
        ...
        if weak_entries.difference(strong_entries):
            yield pvalue.TaggedOutput(self.TAG_ERR, f'Firebase needs {email=}')

        if strong_entries.difference(weak_entries):
            yield pvalue.TaggedOutput(self.TAG_ERR, f'Oppia needs {email=}')

        if weak_entries.intersection(strong_entries):
