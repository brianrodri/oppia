...
describe('Exploration Statistics', function () {
  ...
  it('should collect after learner enters wrong answers', async function () {
    ...
    await learnerTwo.closeBrowser();
  });

  it('should stop collecting after learner exits early', async function () {
    const learnerThree = await UserFactory.createNewUser(
      'learnerThree',
      'learner_three@example.com'
    );
    await learnerThree.playExploration(explorationId);
    await learnerThree.submitAnswerInTextArea('plus');
    await learnerThree.expectResponseFeedbackToBe('Wrong.');
    await learnerThree.closeBrowser();
  });

  it('should present correct stats to creator', async function () {
    await creator.reloadPage();
    await creator.navigateToStatisticsTab();

    await creator.openCardStats(CARD_NAME.INTRODUCTION);
    await creator.expectCardEnteredTimesToBe(3);
    await creator.closeCardStats();

    await creator.openCardStats(CARD_NAME.FINAL_CARD);
    await creator.expectCardEnteredTimesToBe(2);
    await creator.closeCardStats();
    await creator.closeBrowser();
  });
});
