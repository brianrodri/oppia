...
class MockDatetimeForDatastoreTests(test_utils.GenericTestBase):
    ...
    def test_exception_is_raised_when_passed_wrong_type(self):
        with self.assertRaisesRegexp(Exception, 'mocked_now must be datetime'):
            with cloud_datastore_services.mock_datetime_for_datastore(2020):
                pass

    def test_utcnow_always_returns_provided_datetime(self):
        mocked_now = datetime.datetime(2000, 1, 1)
        with cloud_datastore_services.mock_datetime_for_datastore(mocked_now):
    ...
    def test_model_accepts_mocked_datetime(self):
        ...
        class TestModel(cloud_datastore_services.Model):
            """Simple model for testing."""

            datetime_property = cloud_datastore_services.DateTimeProperty()
...
class TransactionTests(test_utils.GenericTestBase):
    ...
    def test_returns_value_of_callback(self):
        self.assertEqual(cloud_datastore_services.transaction(lambda: 1), 1)
    ...
    def test_returns_none_from_void_callback(self):
        ...
        self.assertIsNone(cloud_datastore_services.transaction(do_nothing))
    ...
    def test_raises_exception_from_callback(self):
        ...
        with self.assertRaisesRegexp(Exception, 'uh-oh!'):
            cloud_datastore_services.transaction(raise_exception)

    def test_returns_value_of_nested_transaction(self):
        with self.assertRaisesRegexp(NotImplementedError, 'may not be nested'):
            cloud_datastore_services.transaction(
                lambda: cloud_datastore_services.transaction(lambda: 1))


class EnforcedPropertyTests(unittest.TestCase):

    def test_string_property_raises_value_error_if_indexed_is_false(self):
        with self.assertRaisesRegexp(NotImplementedError, 'must be indexed'):
            cloud_datastore_services.StringProperty(indexed=False)

    def test_text_property_raises_value_error_if_indexed_is_true(self):
        with self.assertRaisesRegexp(NotImplementedError, 'cannot be indexed'):
            cloud_datastore_services.TextProperty(indexed=True)
