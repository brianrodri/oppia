...
def managed_datastore_emulator(silent=True):
    ...
    with contextlib2.ExitStack() as stack:
        if not os.path.exists(DATASTORE_EMULATOR_DATA_DIR):
            os.makedirs(DATASTORE_EMULATOR_DATA_DIR)

        stack.enter_context(managed_process([
            GCLOUD_PATH, 'beta', 'emulators', 'datastore', 'start',
            '--data-dir', DATASTORE_EMULATOR_DATA_DIR,
            '--host-port', '%s:%d' % (
                feconf.DATASTORE_EMULATOR_HOST, feconf.DATASTORE_EMULATOR_PORT),
            '--no-store-on-disk', '--consistency=1.0', '--quiet',
        ]))

        wait_for_port_to_be_open(feconf.DATASTORE_EMULATOR_PORT)

        exports = subprocess.check_output(
            [GCLOUD_PATH, 'beta', 'emulators', 'datastore', 'env-init',
             '--data-dir', DATASTORE_EMULATOR_DATA_DIR, '--quiet'])
