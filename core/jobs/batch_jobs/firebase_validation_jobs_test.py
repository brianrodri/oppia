...
class AuditFirebaseRecordsJobTests(
    ...
    def test_run_with_missing_firebase_record_reports_error(self) -> None:
        # Oppia model exists but no corresponding Firebase user.
        self.put_multi(
            [
                self.create_model(
                    auth_models.UserAuthDetailsModel,
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult(
                    stderr='ERROR: Firebase needs email=\'a@a.com\''
                ),
            ],
        )

    def test_run_with_extra_firebase_record_reports_error(self) -> None:
        # Firebase user exists but no corresponding Oppia model.
        self.firebase_sdk_stub.create_user(
            uid='fb_a', email='a@a.com', disabled=False
        )
        ...
        self.assert_job_output_is(
            [
                job_run_result.JobRunResult(
                    stderr='ERROR: Oppia needs email=\'a@a.com\''
                ),
            ],
        )

    def test_run_with_duplicate_oppia_emails_reports_collision(self) -> None:
        # Two Oppia users with the same email.
        self.put_multi(
            [
                self.create_model(
                    auth_models.UserAuthDetailsModel,
    ...
    def test_run_with_duplicate_firebase_emails_reports_collision(self) -> None:
        # Two Firebase users with the same email.
        self.firebase_sdk_stub.create_user(
            uid='fb_a', email='a@a.com', disabled=False
        )
        self.firebase_sdk_stub.create_user(
    ...
    def test_run_with_mismatched_disabled_reports_both_errors(self) -> None:
        # Oppia says disabled=False but Firebase says disabled=True.
        self.firebase_sdk_stub.create_user(
    ...
    def test_run_with_collision_skips_difference_checks(self) -> None:
        # Two Oppia users with same email AND a Firebase user with same email.
        # Only the collision error should appear, NOT difference errors.
        self.firebase_sdk_stub.create_user(
            uid='fb_a', email='a@a.com', disabled=False
        )
        self.put_multi(
    ...
    def test_run_with_multiple_emails_reports_each(self) -> None:
        # Two separate emails, each matching. Assert OK: 2.
        self.firebase_sdk_stub.create_user(
            uid='fb_a', email='a@a.com', disabled=False
        )
        self.firebase_sdk_stub.create_user(
