...
angular.module('oppia').factory('ThreadDataService', [
  ...
  function(
    ...
    /**
     * Contains all of the threads for this exploration. They are kept as thread
     * domain objects, with a lazily-updated messages property.
     */
    var _threadData = {
      feedbackThreads: [],
      suggestionThreads: [],
    ...
    var _setFeedbackThreadFromBackendDict = function(backendDict) {
      ...
      if (thread === undefined) {
        thread = FeedbackThreadObjectFactory.createFromBackendDict(backendDict);
    ...
    return {
      ...
      fetchThreadSummaries: function() {
        return $http.get(_THREAD_SUMMARY_HANDLER_URL).then(response => {
          _threadSummaryData.feedbackThreads =
            response.data.feedback_thread_summaries.map(
              _setThreadSummaryFromBackendDict);
          _threadSummaryData.suggestionThreads =
            response.data.suggestion_thread_summaries.map(
              _setThreadSummaryFromBackendDict);
          return _threadSummaryData;
        });
      },
      ...
      createNewThread: function(newSubject, newText) {
        return $http.post(_THREAD_LIST_HANDLER_URL, {
          ...
          _openThreadsCount += 1;
          return $q.all([this.fetchThreads(), this.fetchThreadSummaries()]);
        }, () => {
      ...
      markThreadAsSeen: function(threadId) {
        return $http.post(_FEEDBACK_THREAD_VIEW_EVENT_URL + '/' + threadId, {
          thread_id: threadId
        }).then(this.fetchThreadSummaries);
      },
