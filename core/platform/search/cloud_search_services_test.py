...
from core.platform.search import cloud_search_services
...
class SearchAddToIndexTests(test_utils.GenericTestBase):
    ...
    def test_disallow_unsuported_value_types(self):
        with self.assertRaisesRegexp(
            ...
            doc = {'abc': set('xyz')}
            cloud_search_services.add_documents_to_index(doc, 'my_index')
    ...
    def test_add_document_with_existing_id_updates_it(self):
        ...
        cloud_search_services.add_documents_to_index([doc1], 'my_index')
        index = search.Index('my_index')
        self.assertEqual(index.get('doc').field('version').value, 1)
        self.assertEqual(index.get('doc').rank, 10)
        cloud_search_services.add_documents_to_index([doc2], 'my_index')
    ...
    def test_use_default_num_retries(self):
        ...
        failing_put = test_utils.FailingFunction(
            search.Index.put,
            exception,
            cloud_search_services.DEFAULT_NUM_RETRIES,
            )
        ...
        add_docs_ctx = self.swap(
            cloud_search_services,
            'add_documents_to_index',
            add_docs_counter)
        assert_raises_ctx = self.assertRaisesRegexp(
            cloud_search_services.SearchFailureError,
    ...
    def test_use_custom_number_of_retries(self):
        ...
        put_ctx = self.swap(search.Index, 'put', failing_put)
        add_docs_ctx = self.swap(
            cloud_search_services, 'add_documents_to_index', add_docs_counter)
        assert_raises_ctx = self.assertRaisesRegexp(
            cloud_search_services.SearchFailureError,
            '<class \'google.appengine.api.search.search.PutError\'>: lol')
    ...
    def test_put_error_without_transient_result(self):
        ...
        with add_docs_ctx, put_ctx, assert_raises_ctx as e:
            cloud_search_services.add_documents_to_index(docs, 'my_index')
...
class SearchQueryTests(test_utils.GenericTestBase):
    ...
    def test_search_all_documents(self):
        ...
        result = cloud_search_services.search('k:abc', 'my_index')[0]
        self.assertIn({
    ...
    def test_respect_search_query(self):
        ...
        index = search.Index('my_index')
        index.put([doc1, doc2, doc3])
        result = cloud_search_services.search('k:jkl', 'my_index')[0]
        self.assertNotIn({
    ...
    def test_respect_limit(self):
        ...
        index = search.Index('my_index')
        index.put([doc1, doc2, doc3])
        result = cloud_search_services.search('k:abc', 'my_index', limit=2)[0]
    ...
    def test_cursor_is_none_if_no_more_results(self):
        ...
        index = search.Index('my_index')
        index.put([doc1, doc2, doc3])
        cursor = cloud_search_services.search('k:abc', 'my_index')[1]
    ...
    def test_default_rank_is_descending_date(self):
        ...
        cloud_search_services.add_documents_to_index([dict1], 'my_index')
        time.sleep(1)
        cloud_search_services.add_documents_to_index([dict2], 'my_index')
        time.sleep(1)
        cloud_search_services.add_documents_to_index([dict3], 'my_index')
        result = cloud_search_services.search(
            'k:abc', 'my_index', ids_only=True)[0]
    ...
    def test_raise_error_when_sort_starts_with_invalid_character(self):
        ...
        with self.assertRaisesRegexp(
            ...
            cloud_search_services.search('k:abc', 'index', sort=sort_expression)
    ...
    def test_use_default_num_retries(self):
        ...
        search_counter = test_utils.CallCounter(cloud_search_services.search)
        ...
        with search_ctx, search_counter_ctx, assert_raises_ctx as context_mgr:
            cloud_search_services.search('query', 'my_index')
    ...
    def test_use_custom_number_of_retries(self):
        ...
        with index_ctx, search_counter_ctx, assert_raises_ctx:
            cloud_search_services.search('query', 'my_index', retries=3)
    ...
    def test_arguments_are_preserved_in_retries(self):
        ...
        search_counter2 = test_utils.CallCounter(cloud_search_services.search)
...
class ClearIndexTests(test_utils.GenericTestBase):
    def test_clear_index(self):
        ...
        cloud_search_services.clear_index('index')
        result = cloud_search_services.search('k:abc', 'index')[0]
