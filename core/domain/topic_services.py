...
def _create_topic(
    ...
    if does_topic_with_url_fragment_exist(topic.url_fragment):
        raise utils.ValidationError(
            'Topic with URL Fragment \'%s\' already exists' % topic.url_fragment
        )
    ...
    _create_topic(
        ...
        [
            topic_domain.TopicChange(
                {'cmd': topic_domain.CMD_CREATE_NEW, 'name': topic.name}
            )
        ],
    )
...
def apply_change_list(
    ...
    try:
        ...
        logging.error(
            '%s %s %s %s' % (e.__class__.__name__, e, topic_id, change_list)
        )
        raise e
...
def update_topic_and_subtopic_pages(
    ...
    Args:
        ...
        change_list: list(TopicChange, SubtopicPageChange and StudyGuideChange).
    ...
    (
        updated_topic,
        updated_subtopic_pages_dict,
        updated_study_guides_dict,
        deleted_subtopic_ids,
        newly_created_subtopic_ids,
    ...
    if (
        old_topic.url_fragment != updated_topic.url_fragment
        and does_topic_with_url_fragment_exist(updated_topic.url_fragment)
    ):
        raise utils.ValidationError(
...
def publish_story(topic_id: str, story_id: str, committer_id: str) -> None:
    ...
    change_list = [
        topic_domain.TopicChange(
            {'cmd': topic_domain.CMD_PUBLISH_STORY, 'story_id': story_id}
        )
    ]
    _save_topic(
        committer_id,
        topic,
        'Published story with id %s' % story_id,
        change_list,
    )
    generate_topic_summary(topic.id)
...
def unpublish_story(
    ...
    _save_topic(
        committer_id,
        topic,
        'Unpublished story with id %s' % story_id,
        change_list,
    )
    generate_topic_summary(topic.id)
...
def add_canonical_story(user_id: str, topic_id: str, story_id: str) -> None:
    ...
    change_list = [
        topic_domain.TopicChange(
            {'cmd': topic_domain.CMD_ADD_CANONICAL_STORY, 'story_id': story_id}
        )
    ]
    update_topic_and_subtopic_pages(
        user_id,
...
def add_additional_story(user_id: str, topic_id: str, story_id: str) -> None:
    ...
    change_list = [
        topic_domain.TopicChange(
            {'cmd': topic_domain.CMD_ADD_ADDITIONAL_STORY, 'story_id': story_id}
        )
    ]
    update_topic_and_subtopic_pages(
        user_id,
...
def delete_topic(
    ...
    (
        opportunity_services.delete_exploration_opportunities_corresponding_to_topic(
            topic_id
        )
    )
    if feature_flag_services.is_feature_flag_enabled(
...
def compute_summary_of_topic(
    ...
    published_stories = [
        story for story in published_stories_query_result if story is not None
    ]
    ...
    topic_summary = topic_domain.TopicSummary(
        ...
        topic.description,
        topic.version,
        topic_model_canonical_story_count,
        topic_model_additional_story_count,
        topic_model_uncategorized_skill_count,
        topic_model_subtopic_count,
        total_skill_count,
        topic_model_published_node_count,
...
def save_topic_summary(topic_summary: topic_domain.TopicSummary) -> None:
    ...
    Args:
        topic_summary: TopicSummary. The topic summary object to be saved
            in the datastore.
    """
    existing_topic_summary_model = topic_models.TopicSummaryModel.get_by_id(
        topic_summary.id
    )
...
def deassign_manager_role_from_topic(
    committer: user_domain.UserActionsInfo, user_id: str, topic_id: str
) -> None:
    """Deassigns given user from all topics assigned to them.
...
def update_subtopic_thumbnail_filename(
    topic: topic_domain.Topic, subtopic_id: int, new_thumbnail_filename: str
) -> None:
...
def populate_topic_model_fields(
    topic_model: topic_models.TopicModel, topic: topic_domain.Topic
...
def populate_topic_summary_model_fields(
    ...
    topic_summary_dict = {
        ...
        'total_published_node_count': topic_summary.total_published_node_count,
