...
def create_new_question_skill_link(
    user_id: str, question_id: str, skill_id: str, skill_difficulty: float
) -> None:
    ...
    if skill_id not in question.linked_skill_ids:
        ...
        _update_linked_skill_ids_of_question(
            user_id,
            question_id,
            new_linked_skill_ids,
            question.linked_skill_ids,
        )


def update_question_skill_link_difficulty(
    question_id: str, skill_id: str, new_difficulty: float
) -> None:
...
def _update_linked_skill_ids_of_question(
    ...
    (
        opportunity_services.update_skill_opportunities_on_question_linked_skills_change(
...
def get_questions_by_skill_ids(
    ...
    if require_medium_difficulty:
        ...
        question_skill_link_models = question_models.QuestionSkillLinkModel.get_question_skill_links_equidistributed_by_skill(
...
def get_questions_by_ids_and_versions(
    id_version_pairs: List[Tuple[str, int]],
) -> List[question_domain.Question]:
    """Returns historical versions of questions in a single batch lookup.

    Args:
        id_version_pairs: list(tuple(str, int)). A list of
            (question_id, question_version) pairs.

    Returns:
        list(Question). A list of domain objects in the same order as the
        input pairs.

    Raises:
        QuestionSnapshotNotFoundError. If any requested version's snapshot
            does not exist.
    """
    snapshot_ids = [
        question_models.QuestionModel.get_snapshot_id(qid, ver)
        for qid, ver in id_version_pairs
    ]
    snapshots = question_models.QuestionSnapshotContentModel.get_multi(
        snapshot_ids
    )

    results: List[question_domain.Question] = []
    for (question_id, question_version), snapshot in zip(
        id_version_pairs, snapshots
    ):
        if snapshot is None:
            raise QuestionSnapshotNotFoundError(
                'Question snapshot for question %s version %s was not found.'
        ...
        question_model = question_models.QuestionModel(
            id=question_id,
            question_state_data=snapshot.content['question_state_data'],
            language_code=snapshot.content['language_code'],
            version=snapshot.content['version'],
            linked_skill_ids=snapshot.content['linked_skill_ids'],
            inapplicable_skill_misconception_ids=snapshot.content[
                'inapplicable_skill_misconception_ids'
            ],
            next_content_id_index=snapshot.content['next_content_id_index'],
        )
        question_model.question_state_data_schema_version = snapshot.content[
            'question_state_data_schema_version'
        ]
        question_model.created_on = snapshot.created_on
        question_model.last_updated = snapshot.last_updated
        results.append(
...
def get_question_skill_links_of_skill(
    ...
    question_skill_links = [
        get_question_skill_link_from_model(model, skill_description)
        for model in question_models.QuestionSkillLinkModel.get_models_by_skill_id(
            skill_id
        )
    ]
...
def apply_change_list(
    ...
    try:
        for change in change_list:
            if change.cmd == question_domain.CMD_UPDATE_QUESTION_PROPERTY:
                if (
                    ...
                    cmd = cast(
                        question_domain.UpdateQuestionPropertyNextContentIdIndexCmd,
                        change,
                    )
        ...
        logging.error(
            '%s %s %s %s' % (e.__class__.__name__, e, question_id, change_list)
        )
        raise e
...
def untag_deleted_misconceptions(
    ...
    for question in questions_with_none:
        ...
        deleted_inapplicable_skill_misconception_ids = list(
        ...
        answer_groups = list(
            question.question_state_data.interaction.answer_groups
        )
        for answer_group in answer_groups:
            tagged_skill_misconception_id = answer_group.to_dict()[
                'tagged_skill_misconception_id'
            ]
